...
1. **构造全量测试集**（模拟真实座舱用户、以发现问题为导向；禁止写死任何 app/查询；query 首选取自各 capability `description` 的触发场景原句，其余按下列规则组合生成）：
   - **工具覆盖**：目标覆盖全部非 broken capabilities，每个工具 ≥1 条；能力数 >40 时按 domain 分层、每层内按 object 全覆盖；
   - **状态覆盖**：带 enum 的工具取 首/中/尾 三个枚举值各构造一条（值=含义按 description 翻译成口语指令）；readonly 与 normal 能力都测；
   - **必填参数带具体值**：描述中的占位符（X/N/数字）必须替换为具体示例值（enum 优先、描述示例次选）——否则 LLM 合理追问参数，属测试构造缺陷而非描述缺陷（实测教训）；期望 set 类工具的 query 用**绝对值**（"调到8"），相对量指令（"大一点/小一点"）会诱发 LLM 先查后改的合理策略，先查的工具失败（如无车机）会阻断后续 set 造成误判；
   - **复合长难句**：每个主要 domain ≥1 条——一句话多意图（"太吵了调小声点顺便切个安静的模式"）、口语化/带指代/纠偏（"不是这个，换下一个"）、长句噪音（"帮我把那个什么音场弄成驾驶员那边清楚的"）；`expectTool` 填最应命中的主导工具；
   - **近义陷阱**：同 domain 内 object 不同的近义能力对，互相构造易混 query（如整车音量 vs 麦克风音量 vs 切歌），验证 description 区分度；
   - **越界防幻觉**：≥2 条与能力面无关的 query（天气/闲聊/超出范围的请求），`expectTool` 填 `"none"` —— 期望**不调用任何工具**，LLM 纯文本回应即通过；选中任何工具都判失败（幻觉调用）。
2. **分批执行**：`POST $BRIDGE_VIZ_URL/api/e2e/test` body `{"tests":[{"message":"…","expectTool":"<id>|none|省略"},…]}`，**每批 ≤12 条**（同步返回该批逐条判定，通常 1-2 分钟；全量套件分多批顺序提交）。测试开始即自动把用户页面切到 cockpit：每轮自动跟随实时展示，顶部进度条逐条 ✓/✗（悬停见期望 vs 实际选中）——不写管线日志，管线页保持纯净。接口返回 `needsKey` 时页面向用户询问 key，报告说明即可。
3. **以发现问题为导向，失败即自行优化**：某条 ✗（LLM 未选中期望工具 / none 却调用了工具）= 对应 capability 描述区分度不足 → 改写其 description（触发场景更明确、与近义能力可区分），重新导出 function-schema，`POST $BRIDGE_VIZ_URL/api/e2e/restart`（网关按新 analysis 重建配置）后**仅重测失败项**；最多迭代 2 轮。执行类错误（如设备不可达）如实呈现在 cockpit，属环境问题则记录并说明，不算描述失败。
4. 把通过率、覆盖统计（工具数/枚举状态数/复合句数/陷阱数/防幻觉数）、发现的问题与修正记录写进报告。
