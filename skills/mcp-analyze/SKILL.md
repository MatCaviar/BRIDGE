...
## Android/Kotlin+AIDL 基底（额外字段）

当目标是 Android/Kotlin+AIDL app（不是 YunOS agil/D-Bus）时，`app.framework` 设为 `"android-kotlin"`（pipeline 据此分流：scaffold 出 `car-side/registry.json`，`validate_aidl` 跑 Gate A+B）。除通用字段外，每个 capability 还要捕这些字段——它们驱动车端执行器的 dispatch 和源码核对 gate；捕错会被 `validate_aidl` 抓到并反馈给你修。

- **app.nativeCallTool**: 仅当 AIDL 声明了 `callTool(String, IXXXCallback)` 才 `true`。
- **capability.pattern**（读 adapter 方法体，不要猜）:
  - `envelope` — `parseRequest(paramJson)` → 请求信封（如 `ICloudServiceRequest{body,headers,options}`）。tool 只暴露 `body`。
  - `dataclass` — `JsonUtil.fromJson<T>(paramJson)` → 类型化数据类（同时填 `dataClass` = T 名）。
  - `scalar` — `parseJsonObject(paramJson)` + `intArg(...)` 读个别键。
  - `none` — 无参 getter。
- **capability.dataClass**: 仅 dataclass 模式，= `fromJson<T>` 的 T 名（如 `EffectModeAndFB`）。Gate B 校验它存在于 `Types.kt`。
- **capability.devicePaths**: 执行器需注入、agent 无法提供的设备值点路径（如 `["body.vin"]`、`["headers.token"]`）。**每条的叶子段必须在 `app.deviceSources` 里**（`body.vin` 叶子 `vin`），否则 Gate B 报错。设备注入值不要列为 `param`。
- **capability.form**: `binder`（后台 AIDL 直调，返回数据）或 `intent`（start-activity 起页面，fire-and-forget）。
- **capability.status**: stub/返回常量的方法标 `broken`；envelope 方法若云端 `body` schema 源码无法完全核验标 `partial`；否则 `verified`。

Iron Law 不变：上述每个字段都要可溯到 adapter/AIDL/Types 源码。完成后跑 `validate`（schema）+ `validate_aidl`（源码 gate）双绿。
...
- [ ] `framework` is `"YunOS HDT"` (D-Bus) or `"android-kotlin"` (AIDL — then also capture `pattern`/`dataClass`/`devicePaths`/`form`/`nativeCallTool` per the Android section above)
