...
function generateAgentGuide(analysis: AnalysisData): string {
  const lines: string[] = [];
  lines.push(`# Agent Guide: Implementing YunOS Adapter for ${analysis.app.name}`);
  lines.push("");
  lines.push("## Goal");
  lines.push("Implement `src/adapters/yunos-adapter.ts` to replace the mock adapter with real YunOS SDK calls.");
  lines.push("");
  lines.push("## Methods to Implement");
  lines.push("");
  lines.push("| Method | Action | Object | Params | Return Type | Safety | SDK Calls |");
  lines.push("|--------|--------|--------|--------|-------------|--------|-----------|");
  for (const cap of analysis.capabilities) {
    const method = `${cap.action}_${cap.object}`.replace(/_/g, "");
    const params = (cap.params ?? []).map((p) => `${safeFieldName(p.name)}: ${p.type}${p.optional ? "?" : ""}`).join(", ");
    const retType = cap.returns?.type ?? "void";
    lines.push(`| ${method} | ${cap.action} | ${cap.object} | ${params || "none"} | ${retType} | ${cap.safetyLevel} | ${(cap.sdkCalls ?? []).join(", ") || "none"} |`);
  }
  lines.push("");
  lines.push("## SDK Paths");
  lines.push("");
  const sdkPaths = new Set<string>();
  for (const cap of analysis.capabilities) {
    for (const call of cap.sdkCalls ?? []) {
      sdkPaths.add(call);
    }
  }
  ...
  if (safetyGroups.has("p_gear_required")) {
    lines.push("### P-Gear Required");
    lines.push("These methods require the vehicle to be in Park gear:");
    for (const id of safetyGroups.get("p_gear_required") ?? []) {
      lines.push(`- \`${id}\``);
    }
    lines.push("");
  }
  if (safetyGroups.has("p_gear_and_confirm")) {
    lines.push("### P-Gear + User Confirmation");
    lines.push("These methods require P-gear AND explicit user confirmation:");
    for (const id of safetyGroups.get("p_gear_and_confirm") ?? []) {
      lines.push(`- \`${id}\``);
    }
    lines.push("");
  }
  if (safetyGroups.has("p_gear_and_network")) {
    lines.push("### P-Gear + Network");
    lines.push("These methods require P-gear AND active network connection:");
    for (const id of safetyGroups.get("p_gear_and_network") ?? []) {
      lines.push(`- \`${id}\``);
    }
    lines.push("");
  }
  ...
  lines.push("");
  lines.push("1. Start by copying `src/adapters/mock-adapter.ts` as a template");
  lines.push("2. Replace each mock implementation with the actual YunOS SDK call");
  lines.push("3. Update `src/adapters/index.ts` to import and use the YunOS adapter when `mock_mode: false`");
  lines.push("4. Run `npx vitest run` after each method implementation to verify");
  lines.push("5. Ensure all return values match the DTO types defined in `src/adapters/types.ts`");
  return lines.join("\n") + "\n";
}
...
export function scaffoldProject(analysis: AnalysisData, outputDir: string, frameworkPath?: string): void {
  ...
  const files: Array<{ readonly path: string; readonly content: string }> = [
    ...
    { path: "AGENT_GUIDE.md", content: generateAgentGuide(analysis) },
