...
const app = express();
// body 健壮解码: 先原始缓冲, JSON 内容严格 UTF-8、失败按 GBK 重解码(Windows 控制台/curl 常以 GBK 发中文)
...
app.use(((req: express.Request, _res: express.Response, next: express.NextFunction) => {
  ...
  if (ct.includes("application/json") && Buffer.isBuffer(req.body)) {
    let text: string;
    try { text = __utf8Strict.decode(req.body); }
    catch { try { text = __gbkDecoder.decode(req.body); } catch { text = req.body.toString("utf-8"); } }
