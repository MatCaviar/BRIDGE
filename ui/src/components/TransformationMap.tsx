...
export function TransformationMap() {
  ...
  const capabilities = (artifacts?.capabilities ?? []).filter((capability) => capability.selected);
  const tools = new Map((artifacts?.tools ?? []).map((tool) => [tool.name, tool]));
  return <section id="转换图" className="panel glass wide"><div className="panel-title"><div><p className="eyebrow">05 / 溯源场</p><h2>源码 → 能力 → MCP → RPC</h2></div><span className="metric">{capabilities.length} 条链路</span></div><div className="map" role="img" aria-label="源码到 RPC 的转换溯源图"><div className="lane-head"><span>源码</span><span>能力</span><span>MCP 工具</span><span>实车通道</span></div>{capabilities.map((capability) => {
