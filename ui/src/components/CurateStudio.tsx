...
export function CurateStudio() {
  ...
  return <section id="Curate" className="panel glass">
    <div className="panel-title"><div><p className="eyebrow">04 / CURATE STUDIO</p><h2>选择 MCP 化接口</h2></div><span className="metric accent">{selected.size}/{capabilities.length}</span></div>
    ...
    <div className="curate-list">{capabilities.map((capability) => {
      const status = capability.realExecutable ? "REAL READY" : capability.mockExecutable ? "MOCK READY · REAL BLOCKED" : "SOURCE CANDIDATE";
