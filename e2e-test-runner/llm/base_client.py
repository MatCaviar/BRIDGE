...
if TYPE_CHECKING:
    from llm.factory import DictConfig, TaskLog

from llm.util import with_timeout

# Default timeout for LLM API calls (25 minutes)
# Merged dimension analysis (market+talent ~50K tokens) can take 10-15 minutes for GLM-5
DEFAULT_LLM_TIMEOUT_SECONDS = 1500
...
class BaseClient(ABC):
    ...
    def __post_init__(self):
        ...
        self.thinking_mode: Optional[str] = self.cfg.llm.get("thinking_mode")
    ...
    async def create_message(
        ...
        try:
            ...
            # Get stop_reason from response (different for OpenAI vs Anthropic)
            stop_reason = getattr(response, 'stop_reason', None)
            if stop_reason is None and hasattr(response, 'choices'):
                # OpenAI format
                stop_reason = response.choices[0].finish_reason if response.choices else 'N/A'

            self.task_log.log_step(
                "info",
                f"LLM | Call Status",
                f"LLM call status: {stop_reason}, Latency: {latency_ms:.0f}ms"
            )

        except Exception as e:
            error_str = str(e)

            # Check for data inspection failed / inappropriate content errors
            # These should NOT be caught here - let them propagate to pipeline level
            # for proper task restart logic
            if "data_inspection_failed" in error_str or "inappropriate content" in error_str:
                self.task_log.log_step(
                    "error",
                    f"FATAL ERROR | {agent_type} | Content Inspection Error",
                    f"{agent_type} failed due to content inspection: {error_str[:500]}",
                )
                # Re-raise the exception so pipeline can handle task restart
                raise
            ...
            self.task_log.log_step(
                ...
                f"{agent_type} failed: {error_str[:500]}",
            )
            response = None
